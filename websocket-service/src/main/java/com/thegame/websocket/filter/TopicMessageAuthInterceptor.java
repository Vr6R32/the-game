...
public class TopicMessageAuthInterceptor implements ChannelInterceptor {
    ...
    public void handleAuthenticationError(StompHeaderAccessor accessor){
        ...
        messagingTemplate.convertAndSendToUser(
                user.getName(), "/errors", "403 FORBIDDEN"
        );
    }
                ...
                if (xAuthUserJsonObject != null) {
                    ...
                    if (authUserObject != null && authUserObject.accessTokenExpiration() != null) {
                        ...
                        if (authUserObject.accessTokenExpiration().before(new Date())) {
                            log.debug("TOKEN HAS EXPIRED FOR {}", authUserObject.username());
                            handleAuthenticationError(accessor);
                            return null;
                        } else {
                            log.debug("TOKEN IS VALID FOR {}", authUserObject.username());
                        }
                    } else {
                        log.debug("AuthenticationUserObject is null or accessTokenExpiration is null");
                        handleAuthenticationError(accessor);
                        return null;
                    }
                } else {
                    handleAuthenticationError(accessor);
                    log.debug("NO X-AUTH-OBJECT DOESNT SEND MESSAGE {}", accessor.getHost());
                    return null;
                }
            }
            return message;
